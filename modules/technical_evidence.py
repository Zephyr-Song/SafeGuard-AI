"""Deterministic technical evidence builder for SafeBARS.

Runs every seeded protocol case (see ``technical_evidence_cases.py``) through
the framework selector and asserts the workflow behaves as specified in
``research/chi2027/75_revised_contribution_rqs_and_mvp.md``:

* the dual-path routing (human_subjects / ict_research / ai_research) is correct;
* the activated framework set matches the pathway;
* the seeded missing transition is surfaced as ``missing`` rather than hidden;
* every documented/partial dimension links back to at least one source passage;
* repeated runs are deterministic (no silent model variance in this offline path).

This is a *spec-conformance* check, not an ethics-reasoning validation. It runs
fully offline and is intended to be wired into CI so regressions fail the build.

Usage:
    python tests/evaluation/run_technical_evaluation.py [--quiet] [--json-out PATH]
"""

from __future__ import annotations

import argparse
import json
import os
import pathlib
from collections import Counter
from datetime import datetime, timezone
from typing import Any, Dict, List, Tuple

_REPO = pathlib.Path(__file__).resolve().parent.parent

from . import technical_evidence_cases as seed_cases
from .framework_selector import select_framework_path


CHECK_DEFINITIONS = (
    {"id": "pathway", "label": "Correct pathway routing"},
    {"id": "frameworks", "label": "Correct framework activation"},
    {"id": "seeded_missing", "label": "Seeded omission detected"},
    {"id": "passage_refs", "label": "Non-missing outputs link to evidence"},
    {"id": "confidence", "label": "Confidence reported"},
    {"id": "deterministic", "label": "Repeated run is deterministic"},
)


def evaluate_case(case: Dict[str, Any]) -> Dict[str, Any]:
    """Run one seed case and return its checks."""
    project = seed_cases.case_to_project(case)
    passages = case.get("passages", [])

    selection = select_framework_path(project, passages=passages)
    selection_again = select_framework_path(project, passages=passages)

    dims = {d["id"]: d for d in selection["dimensions"]}
    selected_fw = {f["id"] for f in selection["frameworks"]}
    expected_fw = set(case["expected_frameworks"])

    checks: Dict[str, Any] = {}

    # 1. Pathway routing.
    checks["pathway"] = (selection["pathway"] == case["expected_pathway"], selection["pathway"])

    # 2. Activated framework set.
    checks["frameworks"] = (selected_fw == expected_fw, sorted(selected_fw))

    # 3. Seeded missing transition is surfaced, not hidden.
    seeded = case["seeded_missing_dimension"]
    seeded_coverage = dims.get(seeded, {}).get("coverage")
    checks["seeded_missing"] = (seeded_coverage == "missing", seeded_coverage)

    # 4. Passage provenance: documented/partial dims cite a source passage.
    provenance_ok = True
    for d in selection["dimensions"]:
        if d["coverage"] in ("documented", "partial") and not d["source_passage_ids"]:
            provenance_ok = False
            break
    checks["passage_refs"] = (provenance_ok, "ok" if provenance_ok else "missing refs")

    # 5. Confidence is reported.
    checks["confidence"] = (
        bool(selection.get("confidence", {}).get("level")),
        selection.get("confidence", {}).get("level"),
    )

    # 6. Determinism across repeated runs.
    deterministic = json.dumps(selection, sort_keys=True) == json.dumps(
        selection_again, sort_keys=True
    )
    checks["deterministic"] = (deterministic, "ok" if deterministic else "differs")

    coverage_counts = Counter(d["coverage"] for d in selection["dimensions"])
    linked_non_missing = sum(
        1
        for d in selection["dimensions"]
        if d["coverage"] != "missing" and d["source_passage_ids"]
    )
    passed = all(value[0] for value in checks.values())
    return {
        "id": case["id"],
        "title": case["title"],
        "domain": case["domain"],
        "uses_ai": bool(case.get("uses_ai", False)),
        "note": case.get("note", ""),
        "pathway": selection["pathway"],
        "expected_pathway": case["expected_pathway"],
        "passage_count": len(passages),
        "dimension_assessments": len(selection["dimensions"]),
        "coverage_counts": {
            "documented": coverage_counts.get("documented", 0),
            "partial": coverage_counts.get("partial", 0),
            "missing": coverage_counts.get("missing", 0),
        },
        "linked_non_missing_outputs": linked_non_missing,
        "missing_outputs": coverage_counts.get("missing", 0),
        "seeded_missing_dimension": seeded,
        "seeded_missing_detected": checks["seeded_missing"][0],
        "framework_count": len(selected_fw),
        "checks": checks,
        "passed": passed,
    }


def run_all() -> Tuple[List[Dict[str, Any]], Dict[str, Any], int]:
    """Evaluate every seed case. Returns (results, summary, failed_count)."""
    cases = seed_cases.get_seed_cases()
    results = [evaluate_case(c) for c in cases]

    counts = {definition["id"]: 0 for definition in CHECK_DEFINITIONS}
    for r in results:
        for key in counts:
            if r["checks"][key][0]:
                counts[key] += 1

    failed = sum(1 for r in results if not r["passed"])
    domains = {}
    for r in results:
        domains.setdefault(r["domain"], {"total": 0, "passed": 0})
        domains[r["domain"]]["total"] += 1
        domains[r["domain"]]["passed"] += 1 if r["passed"] else 0

    summary = {
        "total_cases": len(results),
        "passed_cases": len(results) - failed,
        "failed_cases": failed,
        "check_totals": {k: {"passed": v, "total": len(results)} for k, v in counts.items()},
        "by_domain": domains,
    }
    return results, summary, failed


def build_public_evidence() -> Dict[str, Any]:
    """Build the public, runtime-computed evidence payload.

    The payload deliberately reports only deterministic specification
    conformance on fictional seeded protocols. It does not read the ignored
    ``results.json`` snapshot and it does not imply human or institutional
    validation.
    """
    results, summary, failed = run_all()
    check_ids = [definition["id"] for definition in CHECK_DEFINITIONS]

    pathway_counts = Counter(r["pathway"] for r in results)
    project_type_counts = Counter("ai" if r["uses_ai"] else "non_ai" for r in results)
    coverage_counts = Counter()
    for result in results:
        coverage_counts.update(result["coverage_counts"])

    assertions_passed = sum(
        1
        for result in results
        for check_id in check_ids
        if result["checks"][check_id][0]
    )
    assertions_total = len(results) * len(check_ids)

    public_cases = []
    for result in results:
        public_cases.append({
            "id": result["id"],
            "title": result["title"],
            "domain": result["domain"],
            "uses_ai": result["uses_ai"],
            "pathway": result["pathway"],
            "passage_count": result["passage_count"],
            "dimension_assessments": result["dimension_assessments"],
            "coverage_counts": result["coverage_counts"],
            "linked_non_missing_outputs": result["linked_non_missing_outputs"],
            "missing_outputs": result["missing_outputs"],
            "seeded_missing_dimension": result["seeded_missing_dimension"],
            "seeded_missing_detected": result["seeded_missing_detected"],
            "framework_count": result["framework_count"],
            "checks_passed": sum(
                1 for check_id in check_ids if result["checks"][check_id][0]
            ),
            "checks_total": len(check_ids),
            "checks": {
                check_id: {
                    "passed": bool(result["checks"][check_id][0]),
                    "observed": result["checks"][check_id][1],
                }
                for check_id in check_ids
            },
            "passed": result["passed"],
        })

    return {
        "metadata": {
            "dataset_label": "Seeded fictional protocol suite (21 cases)",
            "evidence_level": "deterministic_technical_spec_conformance",
            "human_validated": False,
            "commit_sha": next(
                (
                    os.environ[name].strip()
                    for name in (
                        "RENDER_GIT_COMMIT",
                        "GITHUB_SHA",
                        "CF_PAGES_COMMIT_SHA",
                    )
                    if os.environ.get(name, "").strip()
                ),
                None,
            ),
            "boundary_statement": (
                "These results show deterministic specification conformance on "
                "seeded fictional cases. They do not establish ethical correctness, "
                "real-world effectiveness, institutional approval, or validation by "
                "human experts."
            ),
        },
        "generated_at": datetime.now(timezone.utc).isoformat(
            timespec="seconds"
        ).replace("+00:00", "Z"),
        "source": {
            "calculation": "runtime",
            "evaluator": "modules/technical_evidence.py",
            "seed_cases": "modules/technical_evidence_cases.py",
            "method": "offline deterministic evaluator; no LLM calls",
            "ignored_snapshot_required": False,
        },
        "aggregate": {
            "total_cases": summary["total_cases"],
            "passed_cases": summary["passed_cases"],
            "failed_cases": failed,
            "domain_count": len(summary["by_domain"]),
            "domain_counts": summary["by_domain"],
            "pathway_counts": dict(sorted(pathway_counts.items())),
            "project_type_counts": {
                "ai": project_type_counts.get("ai", 0),
                "non_ai": project_type_counts.get("non_ai", 0),
            },
            "total_passages": sum(r["passage_count"] for r in results),
            "dimension_assessments": sum(
                r["dimension_assessments"] for r in results
            ),
            "coverage_counts": {
                "documented": coverage_counts.get("documented", 0),
                "partial": coverage_counts.get("partial", 0),
                "missing": coverage_counts.get("missing", 0),
            },
            "linked_non_missing_outputs": sum(
                r["linked_non_missing_outputs"] for r in results
            ),
            "missing_outputs": sum(r["missing_outputs"] for r in results),
            # Every seed case contains exactly one deliberately omitted
            # dimension; detection is reported separately so a future
            # regression cannot make the injected total appear smaller.
            "seeded_omissions": len(results),
            "detected_seeded_omissions": sum(
                1 for r in results if r["seeded_missing_detected"]
            ),
            "check_type_count": len(CHECK_DEFINITIONS),
            "check_types": [dict(definition) for definition in CHECK_DEFINITIONS],
            "assertions_total": assertions_total,
            "assertions_passed": assertions_passed,
            "assertions_failed": assertions_total - assertions_passed,
            "check_totals": summary["check_totals"],
        },
        "cases": public_cases,
    }


def _print_report(results: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
    print("=" * 72)
    print("SafeBARS Technical Evaluation (offline, deterministic)")
    print("=" * 72)
    for r in results:
        status = "PASS" if r["passed"] else "FAIL"
        print(f"[{status}] {r['id']:<8} {r['domain']:<18} {r['expected_pathway']:<14}")
        if not r["passed"]:
            for key, (ok, got) in r["checks"].items():
                if not ok:
                    print(f"        - {key}: got {got}")
            print(f"        note: {r['note']}")
    print("-" * 72)
    print(f"Cases: {summary['passed_cases']}/{summary['total_cases']} passed "
          f"({summary['failed_cases']} failed)")
    for key, c in summary["check_totals"].items():
        print(f"  {key:<14} {c['passed']}/{c['total']}")
    print("=" * 72)


def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="SafeBARS offline technical evaluation")
    parser.add_argument("--quiet", action="store_true", help="Suppress the per-case report")
    parser.add_argument(
        "--json-out",
        default=str(_REPO / "tests" / "evaluation" / "results.json"),
                        help="Path for JSON results")
    args = parser.parse_args(argv)

    results, summary, failed = run_all()
    if not args.quiet:
        _print_report(results, summary)

    out_path = args.json_out
    try:
        with open(out_path, "w", encoding="utf-8") as fh:
            json.dump({"summary": summary, "results": results}, fh, ensure_ascii=False, indent=2)
    except OSError as exc:  # pragma: no cover - environment dependent
        print(f"Warning: could not write JSON results to {out_path}: {exc}")

    return 1 if failed else 0


if __name__ == "__main__":
    raise SystemExit(main())

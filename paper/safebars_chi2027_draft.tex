...
goal: optimizing the agent to build rapport, trust, and intimacy. We borrow
...
\emph{designs for it multimodally}, on the researcher's \emph{own} artifact,
...
concerns in AI design processes \citep{sadek2023}; AI systems carry systematic,
...
human-subjects review to AI systems \citep{makridis2023}. SafeBARS is our own
...
\includegraphics[width=\columnwidth]{shot_step2_lenses.png}
\caption{Lens selection.}
\end{subfigure}\hfill
\begin{subfigure}{0.48\columnwidth}
\includegraphics[width=\columnwidth]{shot_step3_roles.png}
\caption{Role probes.}
\end{subfigure}

\begin{subfigure}{0.48\columnwidth}
\includegraphics[width=\columnwidth]{shot_step4_map.png}
\caption{Stakeholder map.}
\end{subfigure}\hfill
\begin{subfigure}{0.48\columnwidth}
\includegraphics[width=\columnwidth]{shot_step6_ledger.png}
...
\input{safebars_pilot_evaluation} % Exploratory pilot (n=2, real archived sessions)
\input{safebars_concept_validation} % SIMULATION (N=8 personas) — concept validation, NOT human subjects
...
% ---------------------------------------------------------------------------
\appendix
\section{Supplementary Material: Verbatim Mirror Replay (Pilot R1)}
\label{sec:appendix}

To make the effectiveness claim inspectable rather than asserted, we include a
verbatim replay of researcher R1's SafeBARS mirror session (shared-dorm
fairness assistant). The replay was generated from the archived session JSON
(\texttt{SafeBARS\_StudentTest/study\_bundle.json})---a real capture, not a
live re-run---and renders the mirror's step-by-step surfacing of tensions, the
stakeholder map, and the revision ledger. It substantiates the discovery-yield
counts in Section~\ref{sec:pilot} with the actual conversational sequence
through which the AI brought uncovered ethical gaps to the researcher.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_replay_dorm.pdf}
\caption{Verbatim replay of pilot R1's SafeBARS mirror session (shared-dorm
fairness assistant), generated from the archived session JSON. The mirror
surfaces each tension, renders the stakeholder map, and records the
researcher's revisions---evidence that the tool can surface ethical problems
the researcher did not include in the written plan.}
\label{fig:replay}
\end{figure}
